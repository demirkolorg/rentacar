...
const SagMenu = ({ actions, menuAyar, setMenuAyar }) => {
  const copyToClipboard = async dataToCopy => {
    const copy = menuAyar?.secilenMetin ? menuAyar?.secilenMetin : String(dataToCopy);
  ...
  const genelActions = [
    ...
    },
    {
      ...
      action: data => {
        menuAyar.setGlobalFilter(menuAyar?.secilenMetin ? menuAyar?.secilenMetin : data.value);
        setMenuAyar({ visible: false });
      }
    },
    {
  ...
  return (
    ...
    >
      <Menu>
        <Menu.Item key={`genel`}>
          <p className="flex justify-center bg-slate-900 text-white p-1">Genel İşlemler</p>
        </Menu.Item>
        ...
        <Menu.Item key={`veri`}>
          <p className="flex justify-center bg-slate-900 text-white p-1">Veri İşlemleri</p>
        </Menu.Item>
