...
const Navmenu = ({ menus }) => {
  ...
  useEffect(() => {
    ...
    const childTitle = getChildTitle(locationName);
    document.title = `${childTitle} - ${projectConfig.app.name}`;
                    ...
                    <NavLink to={subItem.childlink}>
                      {({ isActive }) => (
                        <span className={`${isActive ? ' text-black dark:text-white font-medium' : 'text-slate-600 dark:text-slate-300'} text-sm flex space-x-3 items-center transition-all duration-150`}>
                          <span className={`${isActive ? ' bg-slate-900 dark:bg-slate-300 ring-4 ring-opacity-[15%] ring-black-500 dark:ring-slate-300 dark:ring-opacity-20' : ''} h-2 w-2 rounded-full border border-slate-600 dark:border-white inline-block flex-none`}></span>
