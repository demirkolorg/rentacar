      ...
      <Link to="/">
        <div className="flex items-center  space-x-2">
          <div className="logo-icon">
            {!isDark && !isSemiDark ? (
              <img src={MobileLogo} className="h-10 w-10 m-0 p-0 " alt="" />
            ) : (
              <img src={MobileLogoWhite} className="h-10 w-10 m-0 p-0 "  alt="" />
            )}
          </div>

          {(!collapsed || menuHover) && (
            <div>
              <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-100">{projectConfig.app.logoText}</h1>
            </div>
          )}
        </div>
      </Link>
