...
const ProgressBar = ({
  ...
  return (
    <div className="relative">
      {title && <span className={`block text-slate-500   tracking-[0.01em] mb-2 ${titleClass}`}>{title}</span>}
      {
        // if no children, then show the progress bar
        !children && (
          <div className={`w-full  overflow-hidden bg-opacity-10 progress  ${backClass}`}>
      ...
      }
      {
        ...
        children && (
          <div className={`w-full  overflow-hidden bg-opacity-10 flex progress  ${backClass}`}>{children}</div>
        )
      }
    </div>
  );
