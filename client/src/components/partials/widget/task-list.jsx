  ...
  return (
    <div>
      <ul className="divide-y divide-slate-100 dark:divide-slate-700 -mx-6 -mb-6">
        {lists.map(item => (
          <li className="flex items-center px-6 space-x-4 py-4 rtl:space-x-reverse" key={item.id}>
            <div className="flex-none flex space-x-2 items-center rtl:space-x-reverse">
              <Checkbox
                value={item.isDone}
                onChange={() => {
                  setLists(lists.map(list => (list.id === item.id ? { ...list, isDone: !list.isDone } : list)));
                }}
              />
              <div className={`${item.isDone ? ' opacity-20' : ' opacity-100'}  h-8 w-8 rounded-full text-white`}>
                <img src={item.image} alt="" className="block w-full h-full object-cover rounded-full" />
              </div>
            </div>
            <div className={`flex-1 flex  ${item.isDone ? 'line-through dark:text-white' : ''}`}>
